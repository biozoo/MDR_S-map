...
if(do.CCM){ 
  ...
  if(SaveFile){
    ...
    write.csv(ccm.sig,paste('ccm_sig_',da.name,'_nin',nin,'_demo_NEW.csv',sep=''),row.names=F)
    write.csv(ccm.rho,paste('ccm_rho_',da.name,'_nin',nin,'_demo_NEW.csv',sep=''),row.names=F)
  }
}
...
if(CompileCV){
  cv.ind <- NULL; for(alf in 1:nrow(afsp)){
    cv.ind <- rbind(cv.ind,read.csv(paste(da.name,'_nin',nin,'_cvunit',cv.unit,'_alph',alpha.so[afsp[alf,1]]*100,
