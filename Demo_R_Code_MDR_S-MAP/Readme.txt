...
6. Then we reconstructed numerous multiview SSR (recorded in 'eseleLag_XXX') and calulated multiview distance via the function 'mvdist.demo'
...
10.By the R sript, 'NetworkComparison_20210625.R', we can compared the reconstructed interaction Jacobian networks with the theoretical expectation derived from network models (i.e., Fig. 2 in main text)  
...
# Specifically, R codes for model parameterization and computation were included in the file 'TimeSeriesGeneration'.
# The parameters obtained from random simulations were included in the file 'Parameters'; Time series data of population abundance and theoretical interaction Jacobians were in the file, 'Data' 
# R codes for analyzing time series by MDR S-map were included in the file 'MDR_S-MAP' 
# Finally, we can compared the reconstructed interaction Jacobians with their theoretical expectations by executing the R script deposited in the file 'InferenceSkills' based on the results of MDR S-map deposited in the file 'Output'
